...
class Verificatore:
    def __init__(self, id_batch: int) -> None:
        ...
        #struttura dati finale che conterrà il risultato della prov
        self.risultato_verifica: RisultatoVerifica = {
    ...
    def esegui_verifica_integrita(self) -> RisultatoVerifica:
        ...
        - ID Misurazione mancanti → presenti nel file `merkle_paths
         ma assenti nella lista di ID misurazioni ottenuti dal cloud
        - ID Misurazione aggiunti → presenti nella lista di ID misurazioni
        forniti dal cloud ma non previsti nella struttura di ID del file `merkle_paths`
        Nota:
        ...
        e dei Merkle Path, all’ID logico 0 in modo che vengano sempre prima di tutte le misurazioni
        (questo è sempre vero perché gli ID derivano da un campo autoincrement sql che parte da 1)
        """
        id_mancanti, id_aggiunti = self._verifica_struttura()
        self.risultato_verifica["anomalie_strutturali"] = StrutturaVerifica(
        ...
        self.risultato_verifica["anomalie_integrita"] = foglie_anomale

        # STEP 6: Aggiornamento finale del report
        # Si calcola il numero totale di anomalie rilevate (di integrità e strutturali).
        self.risultato_verifica["numero_anomalie_integrita"] = len(foglie_anomale)
        self.risultato_verifica["numero_anomalie_strutturali"] = len(id_mancanti) + len(id_aggiunti)

        # STEP 7: Restituzione dell'oggetto
        return self.risultato_verifica
        ...
        for foglia_id, foglia_hash in self.mappa_id_hash.items():
            ...
            if foglia_id not in self.merkle_paths:
                anomalie[chiave_anomalia] = DettagliVerifica(
                    tipo=tipo,
                    esito=False,
                    modifica_strutturale=True, #modifica strutturale agli ID
            ...
            record = DettagliVerifica(
                ...
                modifica_strutturale=False, #manomissione non strutturale ma a livello di dato
    ...
    def ottieni_numero_anomalie_integrita(self) -> int:
        return self.risultato_verifica["numero_anomalie_integrita"]

    def ottieni_numero_anomalie_strutturali(self) -> int:
        return self.risultato_verifica["numero_anomalie_strutturali"]

    def ottieni_esito_globale(self) -> bool:
        return (self.risultato_verifica["numero_anomalie_integrita"] == 0
                and self.risultato_verifica["numero_anomalie_strutturali"] == 0)

    def batch_alterato(self) -> bool:
        """
        Ritorna True se la prima foglia anomala è il batch (foglia ID 0), False altrimenti.
        """
        anomalie: dict[int, DettagliVerifica] = self.risultato_verifica["anomalie_integrita"]
    ...
    def ottieni_id_misurazioni_alterate(self) -> list[int]:
        """
        Restituisce una lista di ID delle misurazioni alterate a livello di integrità (non strutturalmente).
        Lancia un'eccezione se nessuna misurazione risulta alterata.

        Le misurazioni alterate sono quelle:
        - con tipo 'misurazione'
        - che NON presentano una modifica strutturale (cioè il Merkle Path è valido ma il
    ...
    def _recupera_metadati_anomalie_integrita(self) -> dict:
        """
        ...
        if self.batch_alterato():
            try:
                ...
                metadati_anomalie["metadati_batch"] = metadati_batch
            except Exception as e:
        ...
        if self.misurazioni_alterate():
            try:
                ...
                # Costruisci il dizionario con chiave = id_misurazione,
                # valore = oggetto MetadatiMisurazioneSensorePayload
                metadati_dict = {
                    m.metadati_misurazione.id_misurazione: m.to_dict()
                    for m in metadati_mis_sens
                }
                metadati_anomalie["metadati_misurazioni"] = metadati_dict
            except Exception as e:
    ...
    def ottieni_output_metadati(self) -> tuple[str, str]:
        ...
        metadati_anomalie: dict = self._recupera_metadati_anomalie_integrita()
        return (
            ottieni_report_metadati_anomalie(metadati_anomalie),
            #stringa json in modo pretty (visibile all'essere umano)
            serializza_dict_pretty(metadati_anomalie),
        )

    def ottieni_differenze_anomalie(self) -> tuple[str, str]:
        """
        Versione base: non restituisce differenze per classe Verificatore
        La versione utilizzata dal Verificatore esteso mi consente di ottenere
        la differenza campo per campo delle anomalie per determinare cosa
        è stato modificato
        """
        return "", ""
