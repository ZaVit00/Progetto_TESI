...
Queste classi rappresentano metadati minimali relativi a sensori, misurazioni e tupla batch.
NON fanno parte del flusso regolare di invio dei dati (dal fog node al cloud) ma del flusso
tra Verificatore (leggero) e il cloud. 

Vengono richieste ed elaborate ESCLUSIVAMENTE dal verificatore (leggero) nel momento in cui
una misurazione o una tupla batch viene segnalato come "manomesso", al fine di:
...
class MetadatiMisurazioneSensorePayload(ModelliSerializzabili):
    ...
    #Metodo custom utilizzato per visualizzare l'oggetto composito
