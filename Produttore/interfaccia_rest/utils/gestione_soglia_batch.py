...
# con la struttura binaria del Merkle Tree impostata nel progetto. Difatti l'ultima tupla deve
# essere quella contenente le informazioni sul batch (creato alle xxx, numero di misurazioni y etc)
def aggiorna_soglia_chiusura_batch() -> None:
   ...
   3. Si moltiplica 2^risultato del logaritmo per K_BATCH_SCALING, che è una potenza di due,
      ...
      costruzione del merkle tree (ovviamente ne esiste solo una di tupla del batch)
   4. Si applica un fallback minimo (SOGLIA_BATCH_MINIMA) per evitare batch troppo piccoli.
    ...
    # 1. Ottieni la frequenza media corrente di tutti i sensori registrati nel database
    ...
    #serve ad aumentare l'argomento di log2(x) in modo proporzionale al carico dei sensori.
    ...
    # soglia di fallback evita batch troppo piccoli in caso di pochi sensori inseriti
